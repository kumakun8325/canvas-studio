...
describe('projectService', () => {
  ...
  describe('saveProject', () => {
    it('should save project to Firestore', async () => {
      ...
      // Assert
      expect(doc).toHaveBeenCalledWith(expect.anything(), 'projects', mockProject.id)
  ...
  describe('loadProject', () => {
    it('should load project from Firestore when it exists', async () => {
      ...
      // Assert
      expect(doc).toHaveBeenCalledWith(expect.anything(), 'projects', mockProjectId)
    ...
    it('should handle Firestore errors', async () => {
      ...
      // Act & Assert
      await expect(loadProject(mockProjectId)).rejects.toThrow('Firestore error')
    })
  })
  ...
  describe('deleteProject', () => {
    ...
    it('should handle Firestore errors', async () => {
      ...
      // Act & Assert
      await expect(deleteProject(mockProjectId)).rejects.toThrow('Firestore error')
    })
  })
  ...
  describe('createNewProject', () => {
    ...
    it('should generate unique IDs for each project', async () => {
      ...
      // Act
      const project1 = await createNewProject(mockUserId, 'Project 1', mockTemplate)
      const project2 = await createNewProject(mockUserId, 'Project 2', mockTemplate)

      // Assert
      expect(project1.id).not.toBe(project2.id)
    })

    it('should create project with empty slides array', async () => {
      // Arrange
      vi.mocked(doc).mockReturnValue({} as any)
      vi.mocked(setDoc).mockResolvedValue(undefined)

      // Act
      const result = await createNewProject(mockUserId, 'Empty Project', mockTemplate)
